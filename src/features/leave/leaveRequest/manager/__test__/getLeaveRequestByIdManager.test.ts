...
describe('GET /api/leave-request/manager/:id (Manager)', () => {
  ...
  it('allows MANAGER to get a leave request by ID', async () => {
    ...
    const res = await request(app)
      .get(`/api/leave-request/manager/${leaveRequestId}`)
  ...
  it('rejects if user is not authenticated', async () => {
    const res = await request(app).get(
      `/api/leave-request/manager/${leaveRequestId || fakeId}`,
    );
  ...
  it('rejects if user is not MANAGER', async () => {
    ...
    const res = await request(app)
      .get(`/api/leave-request/manager/${leaveRequestId || fakeId}`)
  ...
  it('returns 404 if leave request does not exist', async () => {
    ...
    const res = await request(app)
      .get(`/api/leave-request/manager/${nonExistentId}`)
      .set('Cookie', managerCookie);
